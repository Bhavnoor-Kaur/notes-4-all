...
def retry_on_exception(
    num_retries=3, on_failure=HttpResponse(status=500), delay_=0.5, backoff_=1.5
):
    def retry(view):
...
class NotesView(View):
    ...
    def get(self, request, id=None, *args, **kwargs):
        ...
        for note in notes:
            res = {
                "id": note["id"],
                "title": note["title"],
                "notes_data": decode_google_cloud(note["note_link"]).decode("ascii"),
            }
            if "summary_link" in note and len(note["summary_link"]) != 0:
                res["summary_data"] = decode_google_cloud(note["summary_link"]).decode(
                    "ascii"
                )
            o_notes.append(res)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "summary_data" in form_data:
            ...
            n = Notes(
                title=name,
                note_link=encode_google_cloud(name, "note", notes_data),
                summary_link=encode_google_cloud(name, "summary", summary_data),
            )
        else:
            n = Notes(
                title=name, note_link=encode_google_cloud(name, "note", notes_data)
            )
        n.save()
    ...
    def put(self, request, *args, **kwargs):
        ...
        if "summary_data" in form_data:
            summary_data = find_summary(form_data["summary_data"])
            curr_note.summary_data = encode_google_cloud(
                curr_note.title, "summary", summary_data
            )
